...
async function checkLateDeliveries() {
  ...
  const endDate = new Date();
  const startDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
  const lateThresholdMs = 5 * 24 * 60 * 60 * 1000; // 5 gün
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error('Trendyol sipariş çekme hatası (' + res.status + ')');
    }
    const data = await res.json();
    const orders = (data && data.content) || [];

    const notifiedSet = new Set(db.getLateDeliveryNotifiedIds());
    const lateDeliveries = [];

    const now = Date.now();
    for (const o of orders) {
      const id = String(o.orderNumber || o.id || '');
      if (!id || notifiedSet.has(id)) continue;

      const orderDate = Number(o.orderDate) || new Date(o.orderDate).getTime() || 0;
      if (!orderDate) continue;

      const ageMs = now - orderDate;
      if (ageMs < lateThresholdMs) continue;

      const status = String(o.status || '').toLowerCase();
      const deliveredStatuses = ['delivered', 'teslim edildi', 'completed', 'tamamlandı', 'shipped', 'kargoya verildi'];
      const isDelivered = deliveredStatuses.some(s => status.includes(s));

      if (!isDelivered) {
        lateDeliveries.push({ id, order: o, ageDays: Math.floor(ageMs / (24 * 60 * 60 * 1000)) });
      }
    }

    if (lateDeliveries.length === 0) {
      return { checked: orders.length, late: 0 };
    }
    ...
    for (const l of lateDeliveries) {
      ...
      try {
        const r = await notifier.notify(
          'TESLİM EDİLEMEDİ (5+ GÜN): ' + l.id,
          '<h3>Kargo 5 günden uzun süredir teslim edilemedi</h3>' + lines.map(ln => '<p>' + ln + '</p>').join(''),
          lines.join('\n')
        );
        if (r && r.telegram && r.telegram.sent) sent++;
      } catch (e) {
        db.addLog('Geç teslim bildirimi gönderilemedi: ' + e.message);
      }
    }

    if (lateDeliveries.length > 0) {
      db.addLog('Geç teslim tespiti: ' + lateDeliveries.length + ' sipariş, ' + sent + ' bildirim gönderildi');
    }
    return { checked: orders.length, late: lateDeliveries.length, sent };
  } catch (e) {
...
module.exports = { syncMarketplace, checkStocks, checkFinancialTransfers, checkQuestions, checkLateDeliveries, syncSharedStock, pushNewProducts, checkOrders, MARKETS, kindLabel, marketConfiguredKinds };
