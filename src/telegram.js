...
let lastAlarmTime = 0;
const ALARM_COOLDOWN_MS = 6 * 60 * 60 * 1000; // 6 saat - bir kere 6 saatte
...
async function healthStatus() {
  ...
  for (const p of products) {
    // Sadece Trendyol stoku kontrolü - diğer pazarlar için "sahte satis" sorunu çözüldü
    const ty = p.trendyolStock;
    if (ty !== null && ty !== undefined && ty <= threshold) {
      issues.push('Trendyol: ' + p.name + ' stok ' + ty + ' (eşik: ' + threshold + ')');
    }
    // Diğer pazarlar alarmsız - sistem sahte satışı önledi (1e3ade3)
  }
  return { ok: issues.length === 0, issues, syncAge: null };
}

async function sendAlarm() {
  const tg = db.getSettings().telegram;
  if (!(tg.enabled && tg.chatId)) return;
  // 6 saatten az önce alarm verildiyse tekrar verme
  if (Date.now() - lastAlarmTime < ALARM_COOLDOWN_MS) {
    db.addLog('Alarm kapalı (cooldown): Son alarm ' + Math.round((Date.now() - lastAlarmTime) / 60000) + ' dk önce verildi');
    return;
  }
  ...
  // Tek seferlik uyarı gönderimi (5 kere skull atma yerine tek mesaj)
  if (detail) {
    for (let i = 0; i < 1; i++) {  // Sadece 1 kere skull gönder
      try {
        await sendTelegramTo(tg.chatId, '\u274C');
      } catch (e) {
        db.addLog('Alarm ikon gönderilemedi: ' + e.message);
      }
      await sleep(3000);
    }
  }
