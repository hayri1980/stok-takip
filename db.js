...
function getLateDeliveryNotifiedIds() {
  load();
  return state.lateDeliveryNotifiedIds || [];
}

function addLateDeliveryNotifiedIds(ids) {
  load();
  const set = new Set(state.lateDeliveryNotifiedIds || []);
  for (const id of ids) set.add(String(id));
  state.lateDeliveryNotifiedIds = Array.from(set).slice(-2000);
  save();
  return state.lateDeliveryNotifiedIds;
}
...
module.exports = {
  ...
  getOrderNotifiedIds,
  addOrderNotifiedIds,
  getLateDeliveryNotifiedIds,
  addLateDeliveryNotifiedIds,
  localDayKey,
  addDailySale,
  getDailySales,
  purgeDailySales,
