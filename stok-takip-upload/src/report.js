...
function buildWeeklyReportText() {
  ...
  const all = db.getDailySales() || [];
  const sales = all.filter(s => String(s.date || '').length === 10 && String(s.date) >= cutoffKey);

  const lines = [];
  lines.push('HAFTALIK OZET — son 7 gun');
  lines.push('--------------------------------');
  let totalQty = 0;
  let totalRevenue = 0;
  let totalDeduct = 0;
  ...
  if (sales.length === 0) {
    ...
    for (const s of sales) {
      ...
      if (rec) {
        ...
        groups.set(key, { qty, revenue, deduct });
      }
    }
    for (const [key, rec] of groups) {
      const parts = key.split('|');
  ...
  lines.push('--------------------------------');
  lines.push('Toplam satis: ' + totalQty + ' adet');
  if (totalRevenue > 0) lines.push('Toplam ciro: ' + Math.round(totalRevenue * 100) / 100 + ' TL');
  if (totalDeduct > 0) {
    lines.push('Maliyet ve kesintiler: ' + Math.round(totalDeduct * 100) / 100 + ' TL');
    lines.push('NET KAR: ' + Math.round((totalRevenue - totalDeduct) * 100) / 100 + ' TL');
  }
...
async function maybeSendWeeklyReport() {
  const rep = db.getSettings().report || {};
  if (rep.enabled === false) return { skipped: true, reason: 'Raporlar kapalı' };

  const now = new Date();
  if (now.getDay() !== 0) return { skipped: true, reason: 'Pazar degil' };

  const date = db.localDayKey(now);
  if (rep.lastWeeklyReportDate === date) return { skipped: true, reason: 'Bu hafta zaten gonderildi' };

  const text = buildWeeklyReportText();
  const subject = 'Haftalik ozet — ' + displayDate(date);
  const html = '<h3>Haftalik satis ozeti</h3><pre>' + escapeHtml(text) + '</pre>';
  const results = await notifier.notify(subject, html, text);
  const sent = (results.email && results.email.sent) || (results.telegram && results.telegram.sent);
  db.addLog('Haftalik ozet gonderildi (' + date + ')' + (sent ? '' : ', GONDERILEMEDI'));
  if (sent) {
    db.setSettings({ report: { ...db.getSettings().report, lastWeeklyReportDate: date } });
  }
  return { date, sent };
}

module.exports = { sendDailyReport, maybeSendDailyReport, maybeSendWeeklyReport, buildDailyReportText };
